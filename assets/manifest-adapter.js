(function attachCuteAssetManifest(global) {
  ...
  function indexManifest(manifest) {
    ...
    return Object.freeze({
      ...
      assetsById,
      recipesById,
      getAsset(id) { return assetsById.get(id) ?? null; },
      getRecipe(id) { return recipesById.get(legacyRecipeAliases[id] ?? id) ?? null; },
      listAssets(type) { return manifest.assets.filter((asset) => !type || asset.type === type); },
      listRecipes(mode) { return manifest.recipes.filter((recipe) => !mode || recipe.mode === mode); },
  ...
  function assertManifestShape(manifest) {
    ...
    if (!Array.isArray(manifest.assets) || !Array.isArray(manifest.recipes)) throw new TypeError("Cute asset manifest must include assets and recipes arrays.");
    const ids = new Set();
  ...
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", installCreativeControls, { once: true });
  else installCreativeControls();
